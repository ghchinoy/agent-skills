...
test("AC8 forward: every key a skill declares reaches its page", async () => {
  ...
  // DENOMINATORS. 29 skills; the exemption fires once per skill and no more,
  // so it is 29 of 29 and cannot have quietly widened to cover a second key.
  assert.equal(skills.length, 29, `swept ${skills.length} skills, not 29`);
  assert.equal(
    exemptSeen,
    skills.length,
...
test("AC8: every rendered field label on all 83 pages traces to a declared key", async () => {
  ...
  assert.equal(pages.length, 83, `swept ${pages.length} pages, not 83`);
