// fields.test.mjs — acceptance criteria 3 and 8, across all 83 pages.
//
...
//   of the spec's six — and it is EMPTY. All 29 skills declare only `name`,
...
//   build actually rendered, on all 83 pages, each must trace to the spec's six,
...
// (pre-registration §6.5). `description` is declared by 29 of 29 skills and is
...
test("AC8 forward: every key a skill declares reaches its page", async () => {
  ...
  // DENOMINATORS. 31 skills; the exemption fires once per skill and no more,
  // so it is 31 of 31 and cannot have quietly widened to cover a second key.
  assert.equal(skills.length, 31, `swept ${skills.length} skills, not 31`);
  assert.equal(
    exemptSeen,
    skills.length,
...
test("AC8: every rendered field label on all 86 pages traces to a declared key", async () => {
  ...
  assert.equal(pages.length, 86, `swept ${pages.length} pages, not 86`);
