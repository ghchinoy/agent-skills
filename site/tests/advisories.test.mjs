...
test("I4: two mention-detectors with different loss profiles select the same set", async () => {
  ...
  // populations in this suite: RESOURCE FILES here (originally 22 + 12 + 24, now 37 + 18 + 30)
  // and CONTENT PAGES in content.test.mjs, in the test named "AC1: dist holds exactly 83
  // content pages", composed 1 + 14 + 29 + 35 + 1 + 3. Different things counted,
  ...
  const RESOURCE_FILE_POPULATION = 85;
  assert.equal(
    population,
    RESOURCE_FILE_POPULATION,
    `the resource FILE population is ${population}; AC1's 37 + 18 + 30 is ` +
      `${RESOURCE_FILE_POPULATION}. This is not the 83 content pages in content.test.mjs.`,
  );
