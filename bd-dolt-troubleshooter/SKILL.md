...
description: Diagnose and repair beads (bd) issue-tracker problems caused by its Dolt backend — especially the "auto-backup failed / table file not found" corruption that silently reverts writes (closes and updates appear to succeed but revert on the next command). Use when bd updates don't persist, issues.jsonl is out of sync with bd show output, or you see Dolt backup/sync errors.
license: Apache-2.0
...
from a mismatch between three layers:

1. **Dolt server** — the live database (`.beads/dolt/`)
...
Run the bundled diagnostic (read-only, safe):
