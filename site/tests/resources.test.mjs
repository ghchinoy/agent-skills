...
test("AC1: the per-group totals are 37 references, 18 scripts and 30 assets — derived", async () => {
  const disk = await resourcesOnDisk();
  const shown = await resourcesRendered();
  const tally = (rows) =>
  ...
  assert.deepEqual(
    tally(disk),
    { references: 37, scripts: 18, assets: 30 },
...
test("AC5: exactly the 18 scripts on disk are linked, at blob URLs, one per file", async () => {
  const disk = (await resourcesOnDisk()).filter((r) => r.group === "scripts");
  const shown = (await resourcesRendered()).filter((r) => r.group === "scripts");

  assert.equal(disk.length, 18, `predicate: files under plugins/*/skills/*/scripts/`);
  assert.equal(shown.length, disk.length);

  const urls = shown.map(hrefOf);
  assert.equal(new Set(urls).size, 18, `the 18 rows produce ${new Set(urls).size} distinct URLs`);
...
test("AC5: what the checker will REQUEST is the same set this suite just asserted", async () => {
  ...
  assert.equal(willRequest.length, 18);
